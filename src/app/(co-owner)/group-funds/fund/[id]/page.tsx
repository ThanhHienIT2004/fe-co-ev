...
export default function FundDetailPage() {
  const { id } = useParams<{ id: string }>(); // ← ĐÚNG: [id]
  ...
  const handleDeposit = async (gateway: 'MOMO' | 'VNPAY') => {
    ...
    try {
      const res = await deposit(Number(id), { amount, gateway });
      if (res.paymentUrl) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100 mb-8">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            />
            <button
              onClick={() => handleDeposit('MOMO')}
              disabled={hookLoading || !depositAmount}
              className="px-5 py-3 bg-gradient-to-r from-pink-500 to-purple-600 text-white rounded-xl font-medium hover:shadow-lg disabled:opacity-70 flex items-center justify-center gap-2"
            >
              MOMO
            </button>
            <button
              onClick={() => handleDeposit('VNPAY')}
              disabled={hookLoading || !depositAmount}
