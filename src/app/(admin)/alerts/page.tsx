...
export default function AlertAdminPage() {
  const { data: alerts, isLoading, error } = useAlerts();

  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      <h1 className="text-2xl font-semibold mb-6">Danh sách cảnh báo</h1>
      ...
      {!isLoading && !error && alerts?.length === 0 && (
        <p className="text-gray-500 text-center py-6">Không có cảnh báo</p>
      )}
            ...
            <tbody>
              {alerts.map(alert => (
                <tr key={alert.alert_id} className="border-t hover:bg-gray-50">
                  <td className="px-4 py-2 text-sm text-gray-700">{alert.alert_id}</td>
                  <td className="px-4 py-2 text-sm text-gray-700">{alert.user_id}</td>
                  ...
                  <td className="px-4 py-2 text-sm text-gray-700">{alert.status}</td>
                  <td className="px-4 py-2 text-sm text-gray-700">{new Date(alert.created_at).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
