...
interface EditMemberFormProps {
  ...
  members: { member_id: number; ownership_ratio: number }[]; // cần có
  groupId: string;
  onClose?: () => void;
  onSubmit?: (data: { group_role?: "admin" | "member"; ownership_ratio?: number }) => void;
}

export default function EditMemberForm({ member, members, onClose, onSubmit }: EditMemberFormProps) {
  ...
  const handleSave = async () => {
    if (ratio < 10 || ratio > 100) {
      enqueueSnackbar("Tỷ lệ sở hữu phải nằm trong 10 - 100%!", { variant: "error" });
      return;
    }

    // Tính tổng các thành viên khác
    const totalOther = members
      .filter((m) => m.member_id !== member.member_id)
      .reduce((sum, m) => sum + m.ownership_ratio, 0);

    const newTotal = totalOther + ratio;

    if (newTotal > 100) {
      enqueueSnackbar(
        `Tổng tỷ lệ sở hữu vượt quá 100%! Hiện tại: ${newTotal}%.`,
        { variant: "error" }
      );
      return;
    }
